...
// Get the whole list of coupons from the Database.
const handler: Handler = async () => {
    try {
        ...
        // Fetch the list of coupons. The newest items sorted to be first in the Array.
        const allCoupons: ICoupon[] | null = await couponsCollection.find({}).sort({createdAt: -1}).toArray();

        return {
            statusCode: 200,
            body: JSON.stringify(allCoupons),
            headers: {
